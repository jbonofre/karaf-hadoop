...
/**
 * This file contains modifications for clean shutdown
 * as the MBeans are not correctly unregistered
 */
...
public class DataNode extends Configured implements InterDatanodeProtocol, ClientDatanodeProtocol, FSConstants, Runnable, DataNodeMXBean {
     ...
     * level of capability
     */
    DataNode(final Configuration conf, final AbstractList<File> dataDirs, SecureDataNodeStarter.SecureResources resources) throws IOException {
        super(conf);
    ...
    void startDataNode(Configuration conf, AbstractList<File> dataDirs, SecureDataNodeStarter.SecureResources resources) throws IOException {
        ...
        * to false on some of them. */
        ...
        HdfsConstants.StartupOption startOpt = getStartupOption(conf);
        ...
        if (simulatedFSDataset) {
            ...
            // constructor below - need to argument ReflectionUtils used below.
    ...
    /**
     * Creates either NID or regular depending on socketWriteTimeout.
     */
    ...
    private NamespaceInfo handshake() throws IOException {
        ...
        while (shouldRun) {
            try {
                ...
                LOG.info("Problem connecting to server: " + getNamenodeAddress());
                try {
                    Thread.sleep(1000);
    ...
    public static InterDatanodeProtocol createInterDataNodeProtocolProxy(DatanodeID datanodeid, final Configuration conf, final int socketTimeout) throws IOException {
        ...
        try {
            return loginUgi.doAs(new PrivilegedExceptionAction<InterDatanodeProtocol>() {
                public InterDatanodeProtocol run() throws Exception {
                    return (InterDatanodeProtocol) RPC.getProxy(InterDatanodeProtocol.class, InterDatanodeProtocol.versionID, addr, conf, socketTimeout);
                }
            });
        } catch (InterruptedException ie) {
    ...
    public static void setNewStorageID(DatanodeRegistration dnReg) {
         ...
         * It is considered extremely rare for all these numbers to match
         * on a different machine accidentally for the following
         * a) SecureRandom(INT_MAX) is pretty much random (1 in 2 billion), and
         * b) Good chance ip address would be different, and
         * c) Even on the same machine, Datanode is designed to use different ports.
         * d) Good chance that these are started at different times.
         * For a conflict to occur all the 4 above have to match!.
        ...
        try {
            ...
            LOG.warn("Could not find ip address of \"default\" interface.");
        }
     ...
     *
     * @see org.apache.hadoop.hdfs.server.namenode.FSNamesystem#registerDatanode(org.apache.hadoop.hdfs.server.protocol.DatanodeRegistration)
     * @throws IOException
     */
    private void register() throws IOException {
        ...
        if (supportAppends) {
            ...
            long[] blockBeingWritten = BlockListAsLongs.convertToArrayLongs(bbwReport);
            namenode.blocksBeingWrittenReport(dnRegistration, blockBeingWritten);
        }
    ...
    private void handleDiskError(String errMsg) {
        ...
        // inform NameNode
        notifyNamenode(dp_error, errMsg);
    ...
    /**
     * Number of concurrent xceivers per nodes.
     */
    int getXceiverCount() {
    ...
    /**
     * Main loop for the DataNode. Runs util shutdown,
        ...
        while (shouldRun) {
            try {
                ...
                if (startTime - lastHeartbeat > heartBeatInterval) {
                    ...
                    // LOG.info("Just sent hearbeat, with name " + localName);
                ...
                if (startTime - lastBlockReport > blockReportInterval) {
                    if (data.isAsyncBlockReportReady()) {
                        ...
                        LOG.info("BlockReport of " + bReport.length + " blocks took " + brCreateCost + " msecs to generate and " +
                ...
                //
                // there is not work to do; sleep until heartbeat timer elapses,
    ...
    private boolean processCommand(DatanodeCommand cmd) throws IOException {
        ...
        switch (cmd.getAction()) {
            ...
            case DatanodeProtocol.DNA_BALANCERBANDWIDTHUPDATE:
                ...
                if (vsn >= 1) {
                    long bandwith = ((BalancerBandwidthCommand) cmd).getBalancerBandwidthValue();
                    if (bandwith > 0) {
                        DataXceiverServer dxcs = (DataXceiverServer) this.dataXceiverServer.getRunnable();
                        dxcs.balanceThrottler.setBandwidth(bandwith);
                    }
                }
                break;
            default:
   ...
   ************************************************************************ */
    ...
    public static DataNode instantiateDataNode(String args[], Configuration conf, SecureDataNodeStarter.SecureResources resources) throws IOException {
        ...
        if (conf.get("dfs.network.script") != null) {
            ...
            return null; // replace for Karaf
            // System.exit(-1); // remove to avoid to shutdown the Karaf container
        }
    ...
    private static boolean parseArguments(String args[], Configuration conf) {
        ...
        for (int i = 0; i < argsLen; i++) {
            ...
            if ("-r".equalsIgnoreCase(cmd) || "--rack".equalsIgnoreCase(cmd)) {
                ...
                // System.exit(-1) // avoid to get out for the JDK in case of container deployment (like Karaf)
                return false;
    ...
    /**
     * This method arranges for the data node to send the block report at the next heartbeat.
     */
     ...
     *
     * @return the fsdataset thats stores the blocks
     */
    ...
    public static void secureMain(String[] args, SecureDataNodeStarter.SecureResources resources) {
        try {
            ...
            // System.exit(-1) // avoid to get out from Java in case of container deployment (like in Karaf)
            return;
        } finally {
            LOG.info("Exiting Datanode");
            // System.exit(0); // avoid to get out from Java in case of container deployment (like in Karaf)
        }
    }
    ...
    private void checkBlockLocalPathAccess() throws IOException {
        ...
        if (!currentUser.equals(this.userWithLocalPathAccess)) {
            throw new AccessControlException("Can't continue with getBlockLocalPathInfo() authorization."
                + " The user " + currentUser + " is not allowed to call getBlockLocalPathInfo");
        }
    }
     ...
     *                   as the block passed in. Otherwise, will calculate the minimum length
    ...
    private LocatedBlock recoverBlock(Block block, boolean keepLength, DatanodeInfo[] targets, boolean closeFile) throws IOException {
        ...
        try {
            ...
            // Number of "replicasBeingWritten" in 0.21 parlance - there are replicas
            ...
            for (DatanodeID id : datanodeids) {
                try {
                    InterDatanodeProtocol datanode = dnRegistration.equals(id) ? this : DataNode.createInterDataNodeProtocolProxy(id, getConf(), socketTimeout);
                    ...
                    if (info.getBlock().getGenerationStamp() < block.getGenerationStamp()) {
                        LOG.info("Only old generation stamp " + info.getBlock().getGenerationStamp() + " found on datanode " + id
                            + " (needed block=" + block + ")");
                        continue;
                    }
    ...
    private LocatedBlock syncBlock(Block block, List<BlockRecord> syncList, DatanodeInfo[] targets, boolean closeFile) throws IOException {
        ...
        // syncList.isEmpty() that all datanodes do not have the block so the block can be deleted.
        if (syncList.isEmpty()) {
    ...
    /**
     * Get current value of the max balancer bandwith in bytes per second.
     *
     * @return bandwith Balancer bandwith in bytes per second for this datanode.
     */
    public Long getBalancerBandwith() {
        DataXceiverServer dxcs = (DataXceiverServer) this.dataXceiverServer.getRunnable();
