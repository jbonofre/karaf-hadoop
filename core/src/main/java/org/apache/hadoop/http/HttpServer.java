...
/**
 * update few parameters to be OSGi compliant when creating the
 * web application (war extraction, direct buffers, ...)
 */
...
public class HttpServer implements FilterContainer {
    ...
    // the ServletContext attribute where the daemon Configuration gets stored.
     ...
     * @param findPort whether the server should start at the given port
     *                 and increment by 1 until it finds a free port.
    ...
    public HttpServer(String name, String bindAddress, int port, boolean findPort, Configuration conf, AccessControlList adminsAcl, Connector connector) throws IOException {
        ...
        defineFilter(webAppContext, "krb5Filter", Krb5AndCertsSslSocketConnector.Krb5SslFilter.class.getName(), null, null);

        addGlobalFilter("safety", org.apache.hadoop.http.HttpServer.QuotingInputFilter.class.getName(), null);
    ...
    protected void addDefaultServlets() {
        ...
        addServlet("stacks", "/stacks", HttpServer.StackServlet.class);
    ...
    public void addJerseyResourcePackage(final String packageName, final String pathSpec) {
        ...
        sh.setInitParameter("com.sun.jersey.config.property.resourceConfigClass", "com.sun.jersey.api.core.PackageResourceConfig");
     ...
     * serlvets added using this method, filters (except internal Kerberized
    ...
    protected String getWebAppsPath() throws IOException {
        // TODO: check if no side effect in OSGi
     ...
     * @deprecated Use {@link #addSslListener(java.net.InetSocketAddress, Configuration, boolean)}
     */
    @Deprecated
    ...
    public void addSslListener(InetSocketAddress addr, Configuration sslConf, boolean needCertsAuth, boolean needKrbAuth) throws IOException {
        ...
        if (needCertsAuth) {
            ...
            System.setProperty("javax.net.ssl.trustStoreType", sslConf.get("ssl.server.truststore.type", ""));
        }
    ...
    public static boolean hasAdministratorAccess(ServletContext servletContext, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (adminsAcl != null) {
            if (!adminsAcl.isUserAllowed(remoteUserUGI)) {
                response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "User " + remoteUser + " is unauthorized to access this page. AccessControlList for accessing this page: " + adminsAcl.toString());
                return false;
            }
        }
        return true;
